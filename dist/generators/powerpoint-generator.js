...
export class PowerPointGenerator {
    ...
    // ============================================================================
    async defineMasterSlide(filename, masterSlide) {
        const pptx = new PptxGenJS();
        // Define slide master
        pptx.defineSlideMaster({
    ...
    async addHyperlinks(filename, slideNumber, links) {
        const pptx = new PptxGenJS();
        const slide = pptx.addSlide();
    ...
    async addSections(filename, sections) {
        const pptx = new PptxGenJS();
    ...
    async addMorphTransition(filename, fromSlide, toSlide, duration) {
        const pptx = new PptxGenJS();
        // Slide 1
        const slide1 = pptx.addSlide();
        ...
        slide2.addText(`Note: Morph transition from slide ${fromSlide} to ${toSlide}\nDuration: ${duration || 1000}ms\n\nRequires PowerPoint 2016+ to apply actual morph effect.`, {
            x: 1,
            y: 5,
            fontSize: 14,
            color: '666666',
    ...
    async addActionButtons(filename, slideNumber, buttons) {
        const pptx = new PptxGenJS();
        const slide = pptx.addSlide();
        ...
        slide.addText('Note: Interactive action buttons require PowerPoint to configure hyperlinks.', {
            x: 0.5,
            y: 6.5,
            fontSize: 12,
            color: '999999',
    ...
    async addSmartArt(args) {
        ...
        slide.addText(`Note: SmartArt placeholder created. Open in PowerPoint to convert to native SmartArt.\nStyle: ${args.smartArt.style || 'flat'} | Color Scheme: ${args.smartArt.colorScheme || 'colorful'}`, {
            x: 0.5,
            y: 6.5,
            fontSize: 11,
            color: '666666',
    ...
    async insertIcons(args) {
        ...
        slide.addText('Note: Icon placeholders created. Open in PowerPoint and use Insert > Icons to replace with actual Microsoft icons.', {
            x: 0.5,
            y: 6.5,
            fontSize: 11,
            color: '666666',
    ...
    async insert3DModels(args) {
        ...
        slide.addText('Note: 3D model placeholders created. Open in PowerPoint and use Insert > 3D Models to add actual .glb/.fbx files.\nAnimation and rotation settings will need to be configured in PowerPoint.', {
            x: 0.5,
            y: 6.5,
            w: '90%',
            fontSize: 11,
    ...
    async addZoom(args) {
        ...
        for (const zoom of args.zooms) {
            ...
            slide.addText(`${zoom.type.toUpperCase()} ZOOM\n${targetInfo}`, {
                x,
                y: y + 0.3,
                w: width,
                h: height - 0.6,
        ...
        slide.addText('Note: Zoom link placeholders created. Open in PowerPoint and use Insert > Zoom to create interactive zoom links.\nRequires PowerPoint 2019 or Microsoft 365.', {
            x: 0.5,
            y: 6.5,
            w: '90%',
            fontSize: 11,
    ...
    async configureRecording(args) {
        ...
        if (args.recording.customArea) {
            settings.push(`Custom Area: ${args.recording.customArea.width}x${args.recording.customArea.height} at (${args.recording.customArea.x}, ${args.recording.customArea.y})`);
        }
        ...
        slide.addText('Note: Recording configuration saved. In PowerPoint, go to Slide Show > Record Slide Show to start recording.\nSettings will need to be configured within PowerPoint recording interface.', {
            x: 0.5,
            y: 6.5,
            w: '90%',
            fontSize: 11,
    ...
    async embedLiveWeb(args) {
        ...
        slide.addText('Note: Live web page placeholders created. In PowerPoint Online (Microsoft 365), use Insert > Video > Online Video or web viewers.\nRequires Microsoft 365 and internet connection during presentation.', {
            x: 0.5,
            y: 6.5,
            w: '90%',
            fontSize: 11,
     ...
     */
    async addCollaborationComments(args) {
        const pptx = new PptxGenJS();
        ...
        summarySlide.addText('Note: Comment metadata saved. Open in PowerPoint and use Review > Comments to view and manage threaded comments.\n@mentions will notify users when the file is shared via OneDrive or SharePoint.', {
            x: 0.5,
            y: 6.5,
            w: '90%',
            fontSize: 11,
     ...
     */
    async configurePresenterCoach(args) {
        const pptx = new PptxGenJS();
        const slide = pptx.addSlide();
        ...
        slide.addText('Note: Presenter Coach settings configured. In PowerPoint (Microsoft 365), go to Slide Show > Rehearse with Coach.\nThe AI coach will provide real-time feedback during rehearsal based on these settings.', {
            x: 0.5,
            y: 6.5,
            w: '90%',
            fontSize: 11,
    ...
    async configureSubtitles(args) {
        ...
        slide.addText('Note: Subtitle configuration saved. In PowerPoint during Slide Show, click "Always Use Subtitles" button or\ngo to Slide Show > Always Use Subtitles. Requires Microsoft 365 and microphone access.', {
            x: 0.5,
            y: 6.5,
            w: '90%',
            fontSize: 11,
    ...
    async addInkAnnotations(args) {
        ...
        for (const annotation of args.annotations) {
            if (annotation.type === 'eraser')
                continue; // Skip eraser for visual representation
        ...
        slide.addText('Note: Ink annotations rendered. In PowerPoint, use Review > Inking or Draw tab to add/edit digital ink.\nPressure sensitivity requires touch-enabled device or stylus.', {
            x: 0.5,
            y: 6.5,
            fontSize: 11,
            color: '666666',
     ...
     */
    async configureGridGuides(args) {
        const pptx = new PptxGenJS();
        const slide = pptx.addSlide();
        ...
        if (args.grid) {
            ...
            if (args.grid.snapToGrid)
                settings.push('  Snap to Grid: ENABLED');
            if (args.grid.spacing)
                settings.push(`  Spacing: ${args.grid.spacing}" intervals`);
            settings.push('');
        }
        // Guide settings
        if (args.guides) {
            ...
            if (args.guides.snapToGuides)
                settings.push('  Snap to Guides: ENABLED');
            settings.push('');
        }
        // Smart guides
        ...
        slide.addText('Note: Grid and guide settings saved. In PowerPoint, go to View > Show > Grid/Guides to toggle display.\nUse View > Guides > Add Vertical/Horizontal Guide to adjust guide positions.', {
            x: 0.5,
            y: 6.8,
            w: '90%',
            fontSize: 11,
     ...
     * @returns Buffer containing the modified presentation
     */
    async manageAnimationPane(args) {
        const pptx = new PptxGenJS();
        const slide = pptx.addSlide();
        ...
        slide.addText('Note: Animation sequence saved. In PowerPoint, go to Animations > Animation Pane to view and manage\nanimation timing, order, and triggers. Drag to reorder, right-click for more options.', {
            x: 0.5,
            y: 6.5,
            w: '90%',
            fontSize: 11,
     ...
     */
    async customizeSlideMaster(args) {
        const pptx = new PptxGenJS();
        ...
        if (args.master.theme?.colors) {
            ...
            const colorKeys = ['accent1', 'accent2', 'accent3', 'accent4', 'accent5', 'accent6'];
        ...
        slide.addText('Note: Slide Master customized. In PowerPoint, go to View > Slide Master to edit layouts.\nAll slides using this master will inherit fonts, colors, effects, and placeholder positions.', {
            x: 0.5,
            y: 6.5,
            w: '90%',
            fontSize: 11,
     ...
     */
    async applyPresentationTheme(args) {
        const pptx = new PptxGenJS();
    ...
    async saveAsTemplate(args) {
        ...
        instructionSlide.addText(`Category: ${args.template.category || 'custom'}`, {
            x: 0.5,
            y: 1.8,
            fontSize: 14,
            color: '999999',
