...
export class WordGenerator {
     ...
     */
    async addBibliography(filename, sources, style = 'APA', insertAt = 'end') {
     ...
     */
    async insertCitations(filename, citations) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
     ...
     */
    async createIndex(filename, entries, title = 'Index', columns = 2, insertAt = 'newPage') {
        await this.loadDocument(filename);
        const children = [];
        if (insertAt === 'newPage') {
            children.push(new Paragraph({ pageBreakBefore: true }));
     ...
     */
    async markIndexEntry(filename, textToMark, entryText, crossReference) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
     ...
     */
    async addFormFields(filename, fields, protectForm = false) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
                text: 'Form Fields',
     ...
     */
    async addContentControls(filename, controls) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
     ...
     */
    async insertSmartArt(filename, smartArt, position) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
     ...
     */
    async addEquations(filename, equations) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
        ...
        equations.forEach((eq, index) => {
            ...
            children.push(new Paragraph({
                ...
                shading: { fill: 'FFF8E1' },
                border: {
                    top: { style: 'single', size: 1, color: 'FFA726' },
                    bottom: { style: 'single', size: 1, color: 'FFA726' },
                    left: { style: 'single', size: 1, color: 'FFA726' },
                    right: { style: 'single', size: 1, color: 'FFA726' },
                },
            }));
        });
        children.push(new Paragraph({
     ...
     */
    async insertSymbols(filename, symbols) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
     ...
     */
    async checkAccessibility(filename, checks, autoFix = false) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
     ...
     */
    async setAltText(filename, altTexts) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
     ...
     */
    async addDigitalSignature(filename, action, certificatePath, password, reason, location) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
     ...
     */
    async protectDocument(filename, protection) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
     ...
     */
    async createMasterDocument(filename, subdocuments, generateTOC = false) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
     ...
     */
    async setDocumentInfo(filename, info) {
        await this.loadDocument(filename);
        const doc = new Document({
            creator: info.author,
            title: info.title,
     ...
     */
    async addCaptions(filename, captions) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
                text: 'Captions',
     ...
     */
    async addAdvancedHyperlinks(filename, hyperlinks) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
     ...
     */
    async addDropCap(filename, paragraphIndex, style, lines = 3, distance = 0) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
     ...
     */
    async addWatermark(filename, watermark) {
        await this.loadDocument(filename);
        const children = [
            new Paragraph({
    ...
    formatCaptionNumber(num, format) {
        switch (format) {
            case 'I, II, III':
